...
for key, value in car_data.items():
    ...
    cr_year, cr_engine_volume, cr_price = search_criteria
...
# Cars should be sorted by price ascending.
# item[1][-1]  - беремо останній елемент в Кортежі (ціна)
sorted_cars = dict(sorted(filtered_cars.items(), key=lambda item: item[1][-1]))
print(sorted_cars)
...
for k, v in sorted_cars.items():
    print(f"{k} {v}")
    counter +=1
    if counter == 5 or counter == len(sorted_cars.items()):
        break